    ...
    if (session) {
      supabase.from('profiles').select('role').eq('id', session.user.id).single()
        .then(({ data }) => {
          ...
          else window.location.replace('/card')
        })
    }
  }, [session])
        ...
        <div style={{ background:black, display:'flex', flexDirection:'column', justifyContent:'center', padding:'5rem', position:'relative', overflow:'hidden' }}>
          ...
          {[['Tarjeta de Lealtad Digital','5 pagos a tiempo = 1 mes gratis.'],['Datos en Tiempo Real','Sellos e historial actualizados al instante.'],['Acceso Seguro','Tu cuenta, tus datos.']].map(([title,sub])=>(
            <div key={title} style={{ display:'flex', alignItems:'flex-start', gap:'1rem', marginBottom:'1.25rem' }}>
              <div style={{ width:28, height:28, borderRadius:'50%', background:'rgba(184,151,90,0.12)', border:'1px solid rgba(184,151,90,0.25)', flexShrink:0 }}></div>
