...
export default function Admin({ session }) {
  ...
  async function loadAll() {
    ...
    const [c,u,r] = await Promise.all([
      apiFetch('/api/admin/cards').then(r=>r.json()),
      apiFetch('/api/admin/users').then(r=>r.json()),
      apiFetch('/api/admin/rewards').then(r=>r.json())
    ])
  ...
  async function doPunch() {
    ...
    const res = await apiFetch('/api/admin/punch', { method:'POST', body: JSON.stringify({ card_id:punchId, payment_amount:punchAmt }) })
  ...
  async function createCard() {
    ...
    const res = await apiFetch('/api/admin/cards', { method:'POST', body: JSON.stringify({ user_id:form.user_id, notes:form.notes }) })
    const data = await res.json()
  ...
  async function deleteCard(id) {
    ...
    await apiFetch('/api/admin/cards', { method:'DELETE', body: JSON.stringify({ id }) })
  ...
  async function saveReward() {
    ...
    const res = await apiFetch('/api/admin/rewards', { method:'POST', body: JSON.stringify({ card_id:card.id, user_id:form.reward_user_id, reward_type:form.reward_type||'1 Mes Gratis', reward_cost:form.reward_cost, notes:form.reward_notes }) })
    if (res.ok) { showToast('Premio registrado'); setModal(null); setForm({}); loadAll() }
  }

  async function deleteReward(id) {
    await apiFetch('/api/admin/rewards', { method:'DELETE', body: JSON.stringify({ id }) })
