  ...
  return (
    <>
      ...
      <div style={{background:'#111110',minHeight:'100vh',fontFamily:ff}}>
        ...
        {/* TOP BAR */}
        <div style={{background:'rgba(14,14,12,0.96)',backdropFilter:'blur(12px)',position:'fixed',top:0,left:0,right:0,zIndex:100,height:52,display:'flex',alignItems:'center',justifyContent:'center',borderBottom:'1px solid rgba(184,151,90,0.1)'}}>
        ...
        <div style={{background:black,padding:'5.5rem 1.25rem 3rem',textAlign:'center',position:'relative',overflow:'hidden'}}>
          ...
          <h2 style={{fontFamily:ffS,fontSize:'clamp(1.6rem,5vw,2.5rem)',fontWeight:300,color:white,marginBottom:'0.4rem'}}>{card.profiles?.full_name||'Bienvenido'}</h2>
          <div style={{fontSize:'0.68rem',color:'rgba(255,255,255,0.28)'}}>{card.profiles?.business_name} · Cliente A+ CRM</div>
        </div>

        {/* CARD */}
        <div style={{maxWidth:420,margin:'0 auto',padding:'0 1.25rem',transform:'translateY(-1.5rem)'}}>
          <div style={{background:'linear-gradient(145deg,#1a1917 0%,#252320 55%,#1a1917 100%)',borderRadius:20,padding:'1.75rem',border:'1px solid rgba(184,151,90,0.28)',boxShadow:'0 30px 70px rgba(0,0,0,0.55)',color:white,position:'relative',overflow:'hidden',marginBottom:'1.25rem'}}>
          ...
          <div style={{textAlign:'center',padding:'2rem 0',borderTop:'1px solid rgba(255,255,255,0.05)',marginTop:'1rem'}}>
            ...
            <div style={{fontSize:'0.52rem',letterSpacing:'0.14em',textTransform:'uppercase',color:'rgba(255,255,255,0.2)',marginTop:'0.25rem'}}>Accounting Plus Client Return Management</div>
          </div>
        </div>
      </div>
    </>
