...
export default function Admin({ session }) {
  ...
  useEffect(() => { if (!session) { router.push('/login'); return }; checkAdmin() }, [session])

  async function checkAdmin() {
    const { data: p } = await supabase.from('profiles').select('role').eq('id', session.user.id).single()
    if (p?.role !== 'admin') { router.push('/card'); return }
    loadAll()
  }
      ...
      <div style={{background:'#f2f0eb',minHeight:'100vh',fontFamily:"'DM Sans',sans-serif"}}>
        ...
        <div style={{display:'flex',paddingTop:58,minHeight:'100vh'}}>
          ...
          <div style={{marginLeft:200,flex:1,padding:'2.5rem',maxWidth:980}}>
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'1rem',marginBottom:'2rem'}}>
              {[[cards.length,'Tarjetas Activas'],[totalStamps,'Sellos Otorgados'],[withReward,'Premios Disponibles'],[redeemed,'Premios Canjeados']].map(([val,label])=>(
                <div key={label} style={{background:white,borderRadius:8,padding:'1.3rem',border:'1px solid rgba(14,14,12,0.06)'}}>
                  <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'2rem',fontWeight:300,color:gold,lineHeight:1}}>{val}</div>
                  ...
                  return (
                    <div key={card.id} style={{background:white,borderRadius:10,border:'1px solid rgba(14,14,12,0.07)',overflow:'hidden'}}>
                      <div style={{background:'linear-gradient(135deg,#1a1917,#252320)',padding:'1.2rem',color:white}}>
                        <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'0.95rem',marginBottom:'0.15rem'}}>A<span style={{color:gold,fontStyle:'italic'}}>+</span> CRM</div>
            ...
            {panel==='punch' && <>
              <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.75rem',fontWeight:300,marginBottom:'1.75rem'}}>Ponchar Tarjeta</h2>
              <div style={{background:white,borderRadius:10,padding:'2rem',border:'1px solid rgba(14,14,12,0.07)'}}>
                ...
                {punchId && (()=>{
                  ...
                  return <div style={{background:'linear-gradient(135deg,#1a1917,#252320)',borderRadius:12,padding:'1.25rem',marginBottom:'1rem',border:'1px solid rgba(184,151,90,0.22)',color:white}}>
                    <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.1rem',marginBottom:'0.5rem'}}>A<span style={{color:gold,fontStyle:'italic'}}>+</span> CRM · {card?.profiles?.business_name||card?.profiles?.full_name}</div>
            ...
            {panel==='rewards' && <>
              ...
              <div style={{background:white,borderRadius:10,overflow:'hidden',border:'1px solid rgba(14,14,12,0.07)'}}>
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  <thead><tr>{['#','Cliente','Tipo de Premio','Costo','Fecha','Estatus',''].map(h=><th key={h} style={{padding:'0.7rem 1.5rem',textAlign:'left',fontSize:'0.56rem',letterSpacing:'0.14em',textTransform:'uppercase',color:gray,borderBottom:'1px solid rgba(14,14,12,0.06)',fontWeight:400}}>{h}</th>)}</tr></thead>
                  <tbody>
                    {rewards.map((r,i)=>(
                      <tr key={r.id}>
                        ...
                        <td style={{padding:'0.95rem 1.5rem',borderBottom:'1px solid rgba(14,14,12,0.04)'}}><button onClick={()=>deleteReward(r.id)} style={{background:'none',border:'none',cursor:'pointer',color:gray,fontSize:'0.75rem'}}>✕</button></td>
                      </tr>
                    ))}
                    {rewards.length===0 && <tr><td colSpan={7} style={{padding:'2rem',textAlign:'center',color:gray,fontSize:'0.82rem'}}>Sin premios registrados aún.</td></tr>}
                  </tbody>
                </table>
              </div>
            </>}
        ...
        {modal==='card' && (
          <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.6)',zIndex:500,display:'flex',alignItems:'center',justifyContent:'center',backdropFilter:'blur(4px)'}} onClick={e=>e.target===e.currentTarget&&setModal(null)}>
            <div style={{background:white,borderRadius:12,padding:'2.25rem',width:480,maxWidth:'92vw',position:'relative'}}>
              <button onClick={()=>setModal(null)} style={{position:'absolute',top:'1.2rem',right:'1.2rem',background:'none',border:'none',fontSize:'1.1rem',cursor:'pointer',color:gray}}>✕</button>
              <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.6rem',fontWeight:300,marginBottom:'0.35rem'}}>Nueva Tarjeta</h3>
        ...
        {modal==='reward' && (
          <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.6)',zIndex:500,display:'flex',alignItems:'center',justifyContent:'center',backdropFilter:'blur(4px)'}} onClick={e=>e.target===e.currentTarget&&setModal(null)}>
            <div style={{background:white,borderRadius:12,padding:'2.25rem',width:480,maxWidth:'92vw',position:'relative'}}>
              <button onClick={()=>setModal(null)} style={{position:'absolute',top:'1.2rem',right:'1.2rem',background:'none',border:'none',fontSize:'1.1rem',cursor:'pointer',color:gray}}>✕</button>
              <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.6rem',fontWeight:300,marginBottom:'0.35rem'}}>Registrar Premio</h3>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'0.85rem'}}>
                <div><label style={lbl}>Tipo</label><select value={form.reward_type||'1 Mes Gratis'} onChange={e=>upd('reward_type',e.target.value)} style={{...inp,marginBottom:0}}>{['1 Mes Gratis','Descuento 50%','Servicio Extra','Consultoría Gratis','Otro'].map(t=><option key={t}>{t}</option>)}</select></div>
        ...
        {toast && <div style={{position:'fixed',bottom:'2rem',right:'2rem',background:black,color:white,padding:'0.95rem 1.4rem',borderRadius:8,fontSize:'0.76rem',boxShadow:'0 8px 30px rgba(0,0,0,0.2)',borderLeft:`3px solid ${gold}`,zIndex:9999,maxWidth:320}}>{toast}</div>}
      </div>
    </>
  )
}
