...
function ColdCallsPanel({ showToast }) {
  const [leads, setLeads] = useState([])
  const [loading, setLoading] = useState(true)
  const [filter, setFilter] = useState('all')
  const [search, setSearch] = useState('')
  const [selected, setSelected] = useState(null)
  const [detailTab, setDetailTab] = useState('info') // 'info' | 'scraper'
  ...
  const emptyForm = {
    ...
    address:'', industry:'', notes:'', call_status:'pending',
    follow_up_date:'', google_maps_url:'', instagram_url:''
  }
  const [form, setForm] = useState(emptyForm)

  const CC_STATUSES = [
    { key:'pending',        label:'Pending',        color:'#6b6b67' },
    { key:'no_answer',      label:'No Answer',      color:'#e67e22' },
    { key:'answered',       label:'Answered',       color:'#3498db' },
    { key:'call_back',      label:'Call Back',      color:'#8e44ad' },
    { key:'follow_up',      label:'Follow Up',      color:'#b8975a' },
    { key:'not_interested', label:'Not Interested', color:'#c0392b' },
    { key:'booked',         label:'Booked ✓',       color:'#2d8a60' },
  ]

  function stOf(s) {
    return CC_STATUSES.find(x=>x.key===s) || { key:s, label:s||'Pending', color:'#6b6b67' }
  }
  ...
  async function load() {
    setLoading(true)
    const res = await fetch('/api/admin/cold-calls')
    const data = await res.json()
    setLeads(data.leads || [])
    setLoading(false)
  }
  ...
  async function saveLead() {
    ...
    const data = await res.json()
    if (data.error) { showToast('Error: '+data.error); setSaving(false); return }
    showToast(form.id ? 'Lead updated' : 'Lead added')
    setSaving(false)
    setShowForm(false)
    setEditMode(false)
    if (data.lead) {
      setLeads(ls => form.id ? ls.map(l=>l.id===data.lead.id?data.lead:l) : [data.lead,...ls])
  ...
  async function updateField(id, updates) {
    ...
    const data = await res.json()
    if (data.lead) {
      setLeads(ls => ls.map(l=>l.id===data.lead.id?data.lead:l))
      setSelected(data.lead)
    }
    return data.lead
  }
  ...
  async function updateStatus(lead, status) {
    await updateField(lead.id, { call_status: status })
    showToast('Status: '+stOf(status).label)
  }

  async function saveNotes() {
    setSavingNotes(true)
    await updateField(selected.id, { notes: notesDraft })
  ...
  async function deleteLead(id) {
    ...
    await fetch('/api/admin/cold-calls', {
  ...
  async function doScrape() {
    ...
    setScraping(true)
    try {
      const res = await fetch('/api/admin/ai-scrape', {
      ...
      if (data.business_name && !form.business_name) setForm(f=>({...f, business_name:data.business_name}))
      if (data.phones?.[0] && !form.phone) setForm(f=>({...f, phone:data.phones[0]}))
      if (data.emails?.[0] && !form.email) setForm(f=>({...f, email:data.emails[0]}))
      if (data.address && !form.address) setForm(f=>({...f, address:data.address}))
      if (data.type==='google_maps' && selected) {
        await updateField(selected.id, { google_maps_url: scrapeUrl.trim() })
      }
      setScrapeUrl('')
      showToast('Link analyzed!')
    } catch { showToast('Could not analyze URL') }
    setScraping(false)
  }

  function openNew() {
    setForm(emptyForm); setScraped([]); setScrapeUrl(''); setEditMode(false); setShowForm(true)
  }
  function openEdit(lead) {
    setForm({ ...lead }); setScraped([]); setScrapeUrl(''); setEditMode(true); setShowForm(true)
  }
  function openDetail(lead) {
    setSelected(lead); setNotesDraft(lead.notes||''); setDetailTab('info'); setScraped([])
  }
  function closeDetail() { setSelected(null); setShowForm(false); setEditMode(false) }

  const today = new Date().toDateString()
  const calledToday = leads.filter(l=>l.updated_at&&new Date(l.updated_at).toDateString()===today&&l.call_status!=='pending').length
  const booked = leads.filter(l=>l.call_status==='booked').length
  const followUpsDue = leads.filter(l=>l.follow_up_date&&new Date(l.follow_up_date+'T23:59:59')<=new Date()&&!['booked','not_interested'].includes(l.call_status)).length
  const answered = leads.filter(l=>['answered','booked'].includes(l.call_status)).length
  const answerRate = leads.length>0 ? Math.round(answered/leads.length*100) : 0

  const filtered = leads.filter(l=>{
    if (filter!=='all'&&l.call_status!==filter) return false
    const q = search.toLowerCase()
    if (!q) return true
    return (l.business_name||'').toLowerCase().includes(q)||
      (l.contact_name||'').toLowerCase().includes(q)||
      (l.phone||'').includes(q)||(l.industry||'').toLowerCase().includes(q)
  })

  // Styles
  const inp3 = { width:'100%', padding:'0.65rem 0.85rem', border:'1px solid '+gl, borderRadius:4, fontFamily:ff, fontSize:'0.82rem', outline:'none', boxSizing:'border-box', background:white, color:black, marginBottom:'0.75rem' }
  const lbl3 = { display:'block', fontSize:'0.55rem', letterSpacing:'0.12em', textTransform:'uppercase', color:gray, marginBottom:'0.3rem' }
  const navBtn = (id) => ({
    padding:'0.4rem 0.85rem', borderRadius:20, border:'none', cursor:'pointer', fontFamily:ff, fontSize:'0.65rem',
    background: detailTab===id ? black : 'rgba(14,14,12,0.06)',
    color: detailTab===id ? white : gray, transition:'all 0.15s'
  })
  ...
  return (
    <div>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'1.5rem'}}>
        <div>
          <h2 style={{fontFamily:ffS,fontSize:'1.5rem',fontWeight:300,marginBottom:'0.15rem'}}>Cold Calls</h2>
          <div style={{fontSize:'0.62rem',color:gray}}>{leads.length} leads · {answerRate}% answer rate</div>
        </div>
        <button onClick={openNew} style={{background:black,color:white,border:'none',padding:'0.6rem 1.1rem',fontFamily:ff,fontSize:'0.6rem',letterSpacing:'0.12em',textTransform:'uppercase',borderRadius:3,cursor:'pointer'}}>+ New Lead</button>
      </div>

      {/* Stats */}
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'0.75rem',marginBottom:'1.5rem'}}>
        {[
          ['Total Leads', leads.length, '#3498db'],
          ['Called Today', calledToday, gold],
          ['Booked', booked, '#2d8a60'],
          ['Follow-ups Due', followUpsDue, followUpsDue>0?'#c0392b':gray],
        ].map(([label,val,color])=>(
          <div key={label} style={{background:white,borderRadius:10,padding:'1.1rem 1.25rem',border:'1px solid rgba(14,14,12,0.07)'}}>
            <div style={{fontFamily:ffS,fontSize:'1.8rem',fontWeight:300,color,lineHeight:1}}>{val}</div>
            <div style={{fontSize:'0.54rem',letterSpacing:'0.1em',textTransform:'uppercase',color:gray,marginTop:'0.4rem'}}>{label}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{display:'flex',gap:'0.4rem',flexWrap:'wrap',marginBottom:'0.85rem'}}>
        <button onClick={()=>setFilter('all')} style={{padding:'0.3rem 0.85rem',borderRadius:20,border:'none',cursor:'pointer',fontFamily:ff,fontSize:'0.62rem',background:filter==='all'?black:'rgba(14,14,12,0.06)',color:filter==='all'?white:gray,transition:'all 0.15s'}}>
          All <span style={{opacity:0.6}}>{leads.length}</span>
        </button>
        {CC_STATUSES.map(s=>(
          <button key={s.key} onClick={()=>setFilter(s.key)}
            style={{padding:'0.3rem 0.85rem',borderRadius:20,border:'none',cursor:'pointer',fontFamily:ff,fontSize:'0.62rem',
              background:filter===s.key?s.color:'rgba(14,14,12,0.06)',
              color:filter===s.key?white:s.color, transition:'all 0.15s'}}>
            {s.label} <span style={{opacity:0.65}}>{leads.filter(l=>l.call_status===s.key).length}</span>
          </button>
        ))}
      </div>
      <input type="text" placeholder="Search by business, name, phone, industry..." value={search} onChange={e=>setSearch(e.target.value)}
        style={{width:'100%',padding:'0.7rem 1rem',border:'1px solid '+gl,borderRadius:3,fontFamily:ff,fontSize:'0.82rem',outline:'none',marginBottom:'1.25rem',boxSizing:'border-box',background:white}}/>

      {/* Leads table */}
      {loading ? (
        <div style={{textAlign:'center',color:gray,padding:'3rem',fontSize:'0.82rem'}}>Loading leads...</div>
      ) : filtered.length===0 ? (
        <div style={{background:white,borderRadius:10,padding:'3rem',textAlign:'center',border:'1px solid rgba(14,14,12,0.07)',color:gray,fontSize:'0.82rem'}}>
          {leads.length===0?'No leads yet — click "+ New Lead" to start tracking calls.':'No leads match this filter.'}
        </div>
      ) : (
        <div style={{background:white,borderRadius:10,border:'1px solid rgba(14,14,12,0.07)',overflow:'hidden'}}>
          <div style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr 0.8fr',padding:'0.6rem 1.25rem',borderBottom:'1px solid rgba(14,14,12,0.06)',fontSize:'0.52rem',letterSpacing:'0.1em',textTransform:'uppercase',color:gray}}>
            <span>Lead</span><span>Status</span><span>Follow-up</span><span style={{textAlign:'right'}}>Action</span>
          </div>
          {filtered.map(lead=>{
            const st = stOf(lead.call_status)
            const overdue = lead.follow_up_date && new Date(lead.follow_up_date+'T23:59:59')<=new Date() && !['booked','not_interested'].includes(lead.call_status)
            const isSelected = selected?.id===lead.id
            return (
              <div key={lead.id} onClick={()=>openDetail(lead)}
                style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr 0.8fr',padding:'0.9rem 1.25rem',borderBottom:'1px solid rgba(14,14,12,0.04)',alignItems:'center',cursor:'pointer',gap:'0.5rem',background:isSelected?'rgba(184,151,90,0.04)':'transparent',transition:'background 0.1s'}}>
                <div style={{minWidth:0}}>
                  <div style={{fontSize:'0.8rem',color:black,fontWeight:500,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{lead.business_name||'Unnamed Lead'}</div>
                  <div style={{fontSize:'0.62rem',color:gray,marginTop:'0.1rem',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
                    {lead.contact_name||''}{lead.contact_name&&lead.phone?' · ':''}{lead.phone||''}{lead.industry?' · '+lead.industry:''}
                  </div>
                </div>
                <span style={{fontSize:'0.6rem',padding:'0.2rem 0.65rem',borderRadius:20,background:st.color+'18',color:st.color,whiteSpace:'nowrap',width:'fit-content'}}>{st.label}</span>
                <div style={{fontSize:'0.65rem',color:overdue?'#c0392b':gray,fontWeight:overdue?600:400}}>
                  {lead.follow_up_date ? new Date(lead.follow_up_date+'T12:00:00').toLocaleDateString('en-US',{month:'short',day:'numeric'}) : '—'}
                  {overdue&&' ⚠'}
                </div>
                <div style={{display:'flex',justifyContent:'flex-end'}}>
                  <button onClick={e=>{e.stopPropagation();openEdit(lead)}} style={{padding:'0.3rem 0.65rem',background:'rgba(14,14,12,0.06)',color:black,border:'none',borderRadius:3,cursor:'pointer',fontFamily:ff,fontSize:'0.56rem',textTransform:'uppercase'}}>Edit</button>
                </div>
              </div>
            )
          })}
      ...
      {selected&&!showForm&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.6)',zIndex:500,display:'flex',alignItems:'flex-end',justifyContent:'center'}}
          ...
          <div style={{background:white,borderRadius:'16px 16px 0 0',width:'100%',maxWidth:680,maxHeight:'92vh',overflowY:'auto'}}>
            {/* Detail Header */}
            <div style={{padding:'1.75rem 2rem 0',background:ink,borderRadius:'16px 16px 0 0',position:'sticky',top:0,zIndex:10}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'1rem'}}>
                <div>
                  <div style={{fontFamily:ffS,fontSize:'1.6rem',fontWeight:300,color:white,marginBottom:'0.2rem'}}>{selected.business_name||'Lead'}</div>
                  <div style={{display:'flex',gap:'0.5rem',flexWrap:'wrap',alignItems:'center'}}>
                    {selected.contact_name&&<span style={{fontSize:'0.68rem',color:'rgba(255,255,255,0.55)'}}>{selected.contact_name}</span>}
                    {selected.industry&&<span style={{fontSize:'0.6rem',color:'rgba(255,255,255,0.35)',borderLeft:'1px solid rgba(255,255,255,0.15)',paddingLeft:'0.5rem'}}>{selected.industry}</span>}
                    <span style={{fontSize:'0.6rem',padding:'0.15rem 0.6rem',borderRadius:20,background:stOf(selected.call_status).color+'28',color:stOf(selected.call_status).color,border:'1px solid '+stOf(selected.call_status).color+'44'}}>{stOf(selected.call_status).label}</span>
                  </div>
                </div>
                <div style={{display:'flex',gap:'0.5rem',flexShrink:0}}>
                  <button onClick={()=>{openEdit(selected)}} style={{padding:'0.45rem 0.9rem',background:'rgba(255,255,255,0.1)',color:white,border:'1px solid rgba(255,255,255,0.2)',borderRadius:4,cursor:'pointer',fontFamily:ff,fontSize:'0.6rem',textTransform:'uppercase'}}>Edit</button>
                  <button onClick={closeDetail} style={{background:'none',border:'none',fontSize:'1.3rem',cursor:'pointer',color:'rgba(255,255,255,0.5)',lineHeight:1,padding:'0.2rem 0.4rem'}}>×</button>
                </div>
              </div>
              {/* Tab Nav */}
              <div style={{display:'flex',gap:'0.25rem',borderTop:'1px solid rgba(255,255,255,0.08)',paddingTop:'0.75rem',paddingBottom:'0'}}>
                {[['info','Call Info'],['scraper','AI Scraper']].map(([id,label])=>(
                  <button key={id} onClick={()=>setDetailTab(id)}
                    style={{padding:'0.45rem 1rem',background:'none',border:'none',cursor:'pointer',fontFamily:ff,fontSize:'0.62rem',letterSpacing:'0.08em',textTransform:'uppercase',
                      color:detailTab===id?gold:'rgba(255,255,255,0.45)',
                      borderBottom:detailTab===id?'2px solid '+gold:'2px solid transparent',
            ...
            <div style={{padding:'1.75rem 2rem'}}>
              ...
              {detailTab==='info'&&(
                <div>
                  ...
                  <div style={{display:'flex',gap:'0.5rem',flexWrap:'wrap',marginBottom:'1.5rem'}}>
                    {selected.phone&&<a href={`tel:${selected.phone}`} style={{display:'flex',alignItems:'center',gap:'0.4rem',padding:'0.5rem 0.9rem',background:'rgba(45,138,96,0.08)',border:'1px solid rgba(45,138,96,0.2)',borderRadius:6,color:'#2d8a60',fontSize:'0.72rem',textDecoration:'none',fontFamily:ff}}>📞 {selected.phone}</a>}
                    {selected.email&&<a href={`mailto:${selected.email}`} style={{display:'flex',alignItems:'center',gap:'0.4rem',padding:'0.5rem 0.9rem',background:'rgba(52,152,219,0.08)',border:'1px solid rgba(52,152,219,0.2)',borderRadius:6,color:'#3498db',fontSize:'0.72rem',textDecoration:'none',fontFamily:ff}}>✉ {selected.email}</a>}
                    {selected.website&&<a href={selected.website} target="_blank" rel="noreferrer" style={{display:'flex',alignItems:'center',gap:'0.4rem',padding:'0.5rem 0.9rem',background:'rgba(142,68,173,0.08)',border:'1px solid rgba(142,68,173,0.2)',borderRadius:6,color:'#8e44ad',fontSize:'0.72rem',textDecoration:'none',fontFamily:ff}}>🌐 Website</a>}
                    {selected.google_maps_url&&<a href={selected.google_maps_url} target="_blank" rel="noreferrer" style={{display:'flex',alignItems:'center',gap:'0.4rem',padding:'0.5rem 0.9rem',background:'rgba(234,67,53,0.08)',border:'1px solid rgba(234,67,53,0.2)',borderRadius:6,color:'#ea4335',fontSize:'0.72rem',textDecoration:'none',fontFamily:ff}}>📍 Maps</a>}
                    {selected.instagram_url&&<a href={selected.instagram_url} target="_blank" rel="noreferrer" style={{display:'flex',alignItems:'center',gap:'0.4rem',padding:'0.5rem 0.9rem',background:'rgba(193,53,132,0.08)',border:'1px solid rgba(193,53,132,0.2)',borderRadius:6,color:'#c13584',fontSize:'0.72rem',textDecoration:'none',fontFamily:ff}}>📸 Instagram</a>}
                    {selected.address&&<span style={{display:'flex',alignItems:'center',gap:'0.4rem',padding:'0.5rem 0.9rem',background:'rgba(14,14,12,0.04)',border:'1px solid rgba(14,14,12,0.1)',borderRadius:6,color:gray,fontSize:'0.72rem',fontFamily:ff}}>📍 {selected.address}</span>}
                  </div>

                  {/* Call Outcome Buttons */}
                  <div style={{marginBottom:'1.5rem'}}>
                    <div style={{fontSize:'0.56rem',letterSpacing:'0.14em',textTransform:'uppercase',color:gray,marginBottom:'0.75rem'}}>Call Outcome</div>
                    <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'0.5rem'}}>
                      {CC_STATUSES.map(s=>{
                        const isActive = selected.call_status===s.key
                        return (
                          <button key={s.key} onClick={()=>updateStatus(selected,s.key)}
                            style={{padding:'0.7rem 0.4rem',border:isActive?`2px solid ${s.color}`:'1px solid rgba(14,14,12,0.1)',borderRadius:8,cursor:'pointer',fontFamily:ff,fontSize:'0.6rem',textTransform:'uppercase',letterSpacing:'0.06em',
                              background:isActive?s.color+'1a':white,color:isActive?s.color:gray,fontWeight:isActive?700:400,transition:'all 0.15s',lineHeight:1.3}}>
                            {s.label}
                          </button>
                        )
                      })}
                  ...
                  {/* Follow-up date */}
                  <div style={{marginBottom:'1.5rem'}}>
                    <label style={lbl3}>Follow-up Date</label>
                    <input type="date" value={selected.follow_up_date||''}
                      onChange={async e=>{ const lead=await updateField(selected.id,{follow_up_date:e.target.value}); if(lead)showToast('Follow-up set') }}
                      style={{padding:'0.65rem 0.85rem',border:'1px solid '+gl,borderRadius:4,fontFamily:ff,fontSize:'0.82rem',outline:'none',background:white,color:black,width:'auto'}}/>
                  </div>

                  {/* Notes */}
                  <div style={{marginBottom:'1.5rem'}}>
                    <label style={lbl3}>Notes</label>
                    <textarea rows={4} value={notesDraft} onChange={e=>setNotesDraft(e.target.value)}
                      placeholder="Call notes, key details, objections..."
                      style={{width:'100%',padding:'0.75rem',border:'1px solid '+gl,borderRadius:4,fontFamily:ff,fontSize:'0.82rem',outline:'none',resize:'vertical',boxSizing:'border-box',color:black,lineHeight:1.6,marginBottom:'0.5rem'}}/>
                    <button onClick={saveNotes} disabled={savingNotes}
                      style={{padding:'0.45rem 1rem',background:black,color:white,border:'none',borderRadius:3,cursor:'pointer',fontFamily:ff,fontSize:'0.6rem',textTransform:'uppercase'}}>
                      {savingNotes?'Saving...':'Save Notes'}
                    </button>
                  </div>

                  {/* Booked Info Section */}
                  {selected.call_status==='booked'&&(
                    <div style={{background:'rgba(45,138,96,0.05)',border:'1px solid rgba(45,138,96,0.2)',borderRadius:10,padding:'1.25rem',marginBottom:'1.5rem'}}>
                      <div style={{fontSize:'0.6rem',letterSpacing:'0.12em',textTransform:'uppercase',color:'#2d8a60',marginBottom:'1rem',display:'flex',alignItems:'center',gap:'0.4rem'}}>
                        ✓ Booked
                        {selected.booked_at&&<span style={{fontWeight:400,color:gray}}>{new Date(selected.booked_at).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'})}</span>}
                      </div>
                      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.75rem'}}>
                        {[
                          ['Google Maps URL','google_maps_url','url','https://maps.app.goo.gl/...'],
                          ['Instagram URL','instagram_url','url','https://instagram.com/...'],
                        ].map(([label,field,type,placeholder])=>(
                          <div key={field}>
                            <label style={lbl3}>{label}</label>
                            <input type={type} defaultValue={selected[field]||''}
                              onBlur={async e=>{ if(e.target.value!==selected[field]) await updateField(selected.id,{[field]:e.target.value}) }}
                              placeholder={placeholder}
                              style={{...inp3,marginBottom:0}}/>
                          </div>
                        ))}
                      </div>
                      <div style={{marginTop:'0.75rem'}}>
                        <label style={lbl3}>Full Address</label>
                        <input type="text" defaultValue={selected.address||''}
                          onBlur={async e=>{ if(e.target.value!==selected.address) await updateField(selected.id,{address:e.target.value}) }}
                          placeholder="123 Main St, Miami FL 33101"
                          style={{...inp3,marginBottom:0}}/>
                      </div>
                      <div style={{marginTop:'0.5rem'}}>
                        <button onClick={()=>setDetailTab('scraper')}
                          style={{padding:'0.5rem 1rem',background:'rgba(52,152,219,0.1)',color:'#3498db',border:'1px solid rgba(52,152,219,0.25)',borderRadius:4,cursor:'pointer',fontFamily:ff,fontSize:'0.62rem',textTransform:'uppercase'}}>
                          ⚡ Open AI Scraper →
                        </button>
                      </div>
                    </div>
                  ...
                  <div style={{borderTop:'1px solid rgba(14,14,12,0.07)',paddingTop:'1rem',display:'flex',justifyContent:'flex-end'}}>
                    <button onClick={()=>deleteLead(selected.id)}
                      style={{padding:'0.45rem 0.85rem',background:'rgba(192,57,43,0.08)',color:'#a93226',border:'none',borderRadius:3,cursor:'pointer',fontFamily:ff,fontSize:'0.6rem',textTransform:'uppercase'}}>
                      Delete Lead
                    </button>
                  </div>
                </div>
              ...
              {detailTab==='scraper'&&(
                <div>
                  <div style={{background:'rgba(52,152,219,0.04)',border:'1px solid rgba(52,152,219,0.15)',borderRadius:10,padding:'1.25rem',marginBottom:'1.5rem'}}>
                    <div style={{fontSize:'0.6rem',letterSpacing:'0.1em',textTransform:'uppercase',color:'#3498db',marginBottom:'0.75rem'}}>⚡ AI Link Analyzer</div>
                    <div style={{fontSize:'0.72rem',color:gray,marginBottom:'0.85rem',lineHeight:1.5}}>
                      Paste a website, Google Maps, or Instagram URL to auto-extract business info, phone numbers, photos, and more.
                    </div>
                    <div style={{display:'flex',gap:'0.5rem'}}>
                      <input type="url" value={scrapeUrl} onChange={e=>setScrapeUrl(e.target.value)}
                        onKeyDown={e=>e.key==='Enter'&&doScrape()}
                        placeholder="https://businesswebsite.com or Google Maps link..."
                        style={{flex:1,padding:'0.65rem 0.85rem',border:'1px solid rgba(52,152,219,0.3)',borderRadius:4,fontFamily:ff,fontSize:'0.78rem',outline:'none',background:white,color:black}}/>
                      <button onClick={doScrape} disabled={scraping}
                        style={{padding:'0.65rem 1.1rem',background:'#3498db',color:white,border:'none',borderRadius:4,cursor:'pointer',fontFamily:ff,fontSize:'0.62rem',textTransform:'uppercase',whiteSpace:'nowrap',opacity:scraping?0.7:1}}>
                  ...
                  {scraped.length===0&&(
                    <div style={{textAlign:'center',color:gray,fontSize:'0.78rem',padding:'2rem 0'}}>
                      No links analyzed yet. Paste a URL above to get started.
                    </div>
                  )}

                  {scraped.map((s,i)=>(
                    <div key={i} style={{background:white,border:'1px solid rgba(14,14,12,0.08)',borderRadius:10,overflow:'hidden',marginBottom:'1rem'}}>
                      {s.type==='error'?(
                        <div style={{padding:'1rem',fontSize:'0.72rem',color:'#c0392b'}}>⚠ Could not fetch: {s.url} — {s.error}</div>
                      ):(
                        <>
                          {s.image&&<img src={s.image} style={{width:'100%',height:160,objectFit:'cover',display:'block'}} onError={e=>e.target.style.display='none'} alt=""/>}
                          <div style={{padding:'1rem'}}>
                            <div style={{fontSize:'0.6rem',color:gray,marginBottom:'0.3rem',textTransform:'uppercase',letterSpacing:'0.08em'}}>{s.type}</div>
                            <div style={{fontFamily:ffS,fontSize:'1.1rem',fontWeight:300,color:black,marginBottom:'0.5rem'}}>{s.business_name||s.title||s.url}</div>
                            {s.description&&<div style={{fontSize:'0.72rem',color:gray,lineHeight:1.6,marginBottom:'0.75rem'}}>{s.description}</div>}
                            <div style={{display:'flex',flexWrap:'wrap',gap:'0.5rem',marginBottom:'0.75rem'}}>
                              {s.phones?.map(p=><a key={p} href={`tel:${p}`} style={{fontSize:'0.7rem',padding:'0.3rem 0.7rem',background:'rgba(45,138,96,0.08)',border:'1px solid rgba(45,138,96,0.2)',borderRadius:20,color:'#2d8a60',textDecoration:'none',fontFamily:ff}}>📞 {p}</a>)}
                              {s.emails?.map(e=><a key={e} href={`mailto:${e}`} style={{fontSize:'0.7rem',padding:'0.3rem 0.7rem',background:'rgba(52,152,219,0.08)',border:'1px solid rgba(52,152,219,0.2)',borderRadius:20,color:'#3498db',textDecoration:'none',fontFamily:ff}}>✉ {e}</a>)}
                              {s.address&&<span style={{fontSize:'0.7rem',padding:'0.3rem 0.7rem',background:'rgba(14,14,12,0.04)',border:'1px solid rgba(14,14,12,0.1)',borderRadius:20,color:gray,fontFamily:ff}}>📍 {s.address}</span>}
                              {s.note&&<span style={{fontSize:'0.7rem',color:gray,fontStyle:'italic'}}>{s.note}</span>}
                            </div>
                            {/* Apply to lead buttons */}
                            <div style={{display:'flex',gap:'0.4rem',flexWrap:'wrap'}}>
                              {s.business_name&&<button onClick={()=>updateField(selected.id,{business_name:s.business_name}).then(()=>showToast('Business name applied'))} style={{padding:'0.3rem 0.65rem',background:'rgba(184,151,90,0.08)',color:gold,border:'1px solid rgba(184,151,90,0.25)',borderRadius:3,cursor:'pointer',fontFamily:ff,fontSize:'0.56rem',textTransform:'uppercase'}}>Apply Name</button>}
                              {s.phones?.[0]&&<button onClick={()=>updateField(selected.id,{phone:s.phones[0]}).then(()=>showToast('Phone applied'))} style={{padding:'0.3rem 0.65rem',background:'rgba(45,138,96,0.08)',color:'#2d8a60',border:'1px solid rgba(45,138,96,0.2)',borderRadius:3,cursor:'pointer',fontFamily:ff,fontSize:'0.56rem',textTransform:'uppercase'}}>Apply Phone</button>}
                              {s.emails?.[0]&&<button onClick={()=>updateField(selected.id,{email:s.emails[0]}).then(()=>showToast('Email applied'))} style={{padding:'0.3rem 0.65rem',background:'rgba(52,152,219,0.08)',color:'#3498db',border:'1px solid rgba(52,152,219,0.2)',borderRadius:3,cursor:'pointer',fontFamily:ff,fontSize:'0.56rem',textTransform:'uppercase'}}>Apply Email</button>}
                              {s.address&&<button onClick={()=>updateField(selected.id,{address:s.address}).then(()=>showToast('Address applied'))} style={{padding:'0.3rem 0.65rem',background:'rgba(14,14,12,0.06)',color:black,border:'none',borderRadius:3,cursor:'pointer',fontFamily:ff,fontSize:'0.56rem',textTransform:'uppercase'}}>Apply Address</button>}
                              {s.type==='google_maps'&&<button onClick={()=>updateField(selected.id,{google_maps_url:s.url}).then(()=>showToast('Maps link saved'))} style={{padding:'0.3rem 0.65rem',background:'rgba(234,67,53,0.08)',color:'#ea4335',border:'1px solid rgba(234,67,53,0.2)',borderRadius:3,cursor:'pointer',fontFamily:ff,fontSize:'0.56rem',textTransform:'uppercase'}}>Save Maps Link</button>}
                              {s.type==='instagram'&&<button onClick={()=>updateField(selected.id,{instagram_url:s.url}).then(()=>showToast('Instagram saved'))} style={{padding:'0.3rem 0.65rem',background:'rgba(193,53,132,0.08)',color:'#c13584',border:'1px solid rgba(193,53,132,0.2)',borderRadius:3,cursor:'pointer',fontFamily:ff,fontSize:'0.56rem',textTransform:'uppercase'}}>Save Instagram</button>}
                            </div>
                          </div>
                        </>
                      )}
      ...
      {showForm&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.6)',zIndex:500,display:'flex',alignItems:'flex-end',justifyContent:'center'}}
          ...
          <div style={{background:white,borderRadius:'16px 16px 0 0',width:'100%',maxWidth:560,maxHeight:'92vh',overflowY:'auto',padding:'2rem'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'1.5rem'}}>
              <h3 style={{fontFamily:ffS,fontSize:'1.4rem',fontWeight:300}}>{editMode?'Edit Lead':'New Lead'}</h3>
              <button onClick={()=>setShowForm(false)} style={{background:'none',border:'none',fontSize:'1.3rem',cursor:'pointer',color:gray}}>×</button>
            </div>

            {/* Quick AI Scraper in form */}
            {!editMode&&(
              <div style={{background:'rgba(52,152,219,0.05)',border:'1px solid rgba(52,152,219,0.15)',borderRadius:8,padding:'1rem',marginBottom:'1.5rem'}}>
                <div style={{fontSize:'0.58rem',letterSpacing:'0.1em',textTransform:'uppercase',color:'#3498db',marginBottom:'0.6rem'}}>⚡ Auto-fill from URL</div>
                <div style={{display:'flex',gap:'0.5rem'}}>
                  <input type="url" value={scrapeUrl} onChange={e=>setScrapeUrl(e.target.value)}
                    onKeyDown={e=>e.key==='Enter'&&doScrape()}
                    placeholder="Paste website or Google Maps link..."
                    style={{flex:1,padding:'0.6rem 0.85rem',border:'1px solid rgba(52,152,219,0.3)',borderRadius:4,fontFamily:ff,fontSize:'0.78rem',outline:'none',background:white,color:black}}/>
                  <button onClick={doScrape} disabled={scraping}
                    style={{padding:'0.6rem 1rem',background:'#3498db',color:white,border:'none',borderRadius:4,cursor:'pointer',fontFamily:ff,fontSize:'0.6rem',textTransform:'uppercase',opacity:scraping?0.7:1}}>
                    {scraping?'...':'Fill'}
                  </button>
                </div>
                {scraped.length>0&&scraped[0].type!=='error'&&(
                  <div style={{marginTop:'0.65rem',padding:'0.6rem 0.85rem',background:white,borderRadius:4,border:'1px solid rgba(14,14,12,0.08)',fontSize:'0.68rem',color:gray}}>
                    ✓ Filled: {[scraped[0].business_name,scraped[0].phones?.[0],scraped[0].emails?.[0]].filter(Boolean).join(' · ')}
                  </div>
                )}
              </div>
            )}
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.75rem'}}>
              <div><label style={lbl3}>Business Name *</label><input style={inp3} type="text" value={form.business_name} onChange={e=>setForm(f=>({...f,business_name:e.target.value}))} placeholder="ABC Hair Salon"/></div>
              <div><label style={lbl3}>Contact Name</label><input style={inp3} type="text" value={form.contact_name} onChange={e=>setForm(f=>({...f,contact_name:e.target.value}))} placeholder="Maria Lopez"/></div>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.75rem'}}>
              <div><label style={lbl3}>Phone</label><input style={inp3} type="tel" value={form.phone} onChange={e=>setForm(f=>({...f,phone:e.target.value}))} placeholder="(555) 123-4567"/></div>
              <div><label style={lbl3}>Email</label><input style={inp3} type="email" value={form.email} onChange={e=>setForm(f=>({...f,email:e.target.value}))} placeholder="owner@business.com"/></div>
            </div>
            <div><label style={lbl3}>Website</label><input style={inp3} type="url" value={form.website||''} onChange={e=>setForm(f=>({...f,website:e.target.value}))} placeholder="https://..."/></div>
            <div><label style={lbl3}>Address</label><input style={inp3} type="text" value={form.address||''} onChange={e=>setForm(f=>({...f,address:e.target.value}))} placeholder="123 Main St, Miami FL"/></div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.75rem'}}>
              <div><label style={lbl3}>Industry</label><input style={inp3} type="text" value={form.industry||''} onChange={e=>setForm(f=>({...f,industry:e.target.value}))} placeholder="Hair Salon, Restaurant..."/></div>
              <div>
                <label style={lbl3}>Status</label>
                <select style={inp3} value={form.call_status} onChange={e=>setForm(f=>({...f,call_status:e.target.value}))}>
                  {CC_STATUSES.map(s=><option key={s.key} value={s.key}>{s.label}</option>)}
                </select>
              </div>
            </div>
            <div><label style={lbl3}>Follow-up Date</label><input style={inp3} type="date" value={form.follow_up_date||''} onChange={e=>setForm(f=>({...f,follow_up_date:e.target.value}))}/></div>
            <div>
              <label style={lbl3}>Notes</label>
              <textarea style={{...inp3,resize:'vertical'}} rows={3} value={form.notes||''} onChange={e=>setForm(f=>({...f,notes:e.target.value}))} placeholder="Call notes, key info, objections..."/>
            </div>
            <div style={{display:'flex',gap:'0.75rem'}}>
              <button onClick={saveLead} disabled={saving}
                style={{flex:1,background:black,color:white,border:'none',padding:'0.85rem',fontFamily:ff,fontSize:'0.66rem',letterSpacing:'0.14em',textTransform:'uppercase',borderRadius:3,cursor:'pointer',opacity:saving?0.7:1}}>
                {saving?'Saving...':(editMode?'Save Changes':'Add Lead')}
              </button>
              <button onClick={()=>setShowForm(false)} style={{background:'rgba(14,14,12,0.06)',color:black,border:'none',padding:'0.85rem 1.25rem',fontFamily:ff,fontSize:'0.66rem',textTransform:'uppercase',borderRadius:3,cursor:'pointer'}}>Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{background:'#f2f0eb',minHeight:'100vh',fontFamily:ff,paddingBottom:70}}>
        ...
        <div style={{display:'flex',paddingTop:52,minHeight:'100vh'}}>
          ...
          <div className="admin-sidebar" style={{width:205,background:ink,flexShrink:0,position:'fixed',top:52,left:0,bottom:0,padding:'1.5rem 0',overflowY:'auto'}}>
            ...
            <button onClick={()=>setPanel('coldcalls')} style={{display:'flex',alignItems:'center',padding:'0.82rem 1.5rem',fontSize:'0.72rem',letterSpacing:'0.1em',textTransform:'uppercase',color:panel==='coldcalls'?gold:'rgba(255,255,255,0.95)',cursor:'pointer',background:'none',border:'none',borderLeft:panel==='coldcalls'?'2px solid '+gold:'2px solid transparent',width:'100%',textAlign:'left',fontFamily:ff}}>Cold Calls</button>
          ...
          <div className="admin-main" style={{marginLeft:205,flex:1,padding:'1.75rem',maxWidth:980}}>
            ...
            {panel==='coldcalls'&&<ColdCallsPanel showToast={showToast}/>}
