...
function MisClientesPanel({userRole,agentId,showToast}){
  const {dm,inkC,grayC,cardBg,cardBorder,inputBg,inputBorder,subtleBg,divider}=useTheme()
  const [clients,setClients]=useState([])
  const [loading,setLoading]=useState(true)
  const [expanded,setExpanded]=useState({})
  ...
  async function load(){
    setLoading(true)
    const r=await fetch('/api/admin/bookings?status=bought')
    const d=await r.json()
    // agents only see their own
    const all=Array.isArray(d)?d:[]
    setClients(userRole==='agent'?all.filter(b=>b.agent_id===agentId):all)
    setLoading(false)
  }
  async function loadFiles(clientId){
    if(files[clientId]) return
    const r=await fetch(`/api/admin/files?user_id=${clientId}`)
    const d=await r.json()
    setFiles(f=>({...f,[clientId]:d.files||[]}))
  }
  function toggle(id){
    const next=!expanded[id]
    setExpanded(e=>({...e,[id]:next}))
    if(next) loadFiles(id)
  }
  async function upload(clientId,file){
    setUploading(u=>({...u,[clientId]:true}))
    const fd=new FormData()
    fd.append('file',file)
    fd.append('user_id',clientId)
    const r=await fetch('/api/admin/files',{method:'POST',body:fd})
    if(r.ok){showToast('Archivo subido');setFiles(f=>({...f,[clientId]:null}));loadFiles(clientId)}
    else showToast('Error al subir')
    setUploading(u=>({...u,[clientId]:false}))
  }
  async function deleteFile(clientId,path){
    if(!confirm('¿Borrar este archivo?')) return
    await fetch('/api/admin/files',{method:'DELETE',headers:{'Content-Type':'application/json'},body:JSON.stringify({path})})
    showToast('Eliminado')
    setFiles(f=>({...f,[clientId]:null}))
    loadFiles(clientId)
  }
  const filtered=clients.filter(c=>`${c.name} ${c.business||''} ${c.service||''}`.toLowerCase().includes(q.toLowerCase()))
  const ff2=`'Inter',sans-serif`
  return(
    <div style={{padding:'20px 16px 100px',fontFamily:ff2}}>
      <h1 style={{fontFamily:`'Cormorant Garamond',serif`,fontSize:26,fontWeight:300,color:inkC,marginBottom:6}}>Mis Clientes</h1>
      <p style={{fontSize:12,color:grayC,marginBottom:16}}>Clientes que cerraron contigo — sube contratos, fotos y archivos</p>
      <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Buscar cliente..." style={{width:'100%',boxSizing:'border-box',height:40,borderRadius:10,border:`1px solid ${inputBorder}`,padding:'0 12px',fontSize:13,fontFamily:ff2,background:inputBg,color:inkC,outline:'none',marginBottom:16}}/>
      {loading&&<p style={{textAlign:'center',color:grayC,padding:'40px 0',fontSize:13}}>Cargando...</p>}
      {!loading&&filtered.length===0&&<p style={{textAlign:'center',color:grayC,padding:'40px 0',fontSize:13}}>Sin clientes cerrados</p>}
      {filtered.map(c=>{
        const open=expanded[c.id]
        const clientFiles=files[c.id]||[]
        return(
          <div key={c.id} style={{background:cardBg,border:`1px solid ${cardBorder}`,borderRadius:14,marginBottom:10,overflow:'hidden'}}>
            <button onClick={()=>toggle(c.id)} style={{width:'100%',display:'flex',justifyContent:'space-between',alignItems:'center',padding:'14px 16px',background:'none',border:'none',cursor:'pointer',touchAction:'manipulation'}}>
              <div style={{textAlign:'left'}}>
                <p style={{fontSize:15,fontWeight:600,color:inkC,margin:0}}>{c.name}</p>
                {c.business&&c.business!==c.name&&<p style={{fontSize:12,color:grayC,margin:'2px 0 0'}}>{c.business}</p>}
                <p style={{fontSize:11,color:grayC,margin:'2px 0 0'}}>{c.buy_service||c.service||'Servicio'}{c.buy_amount?` · $${c.buy_amount}`:''}</p>
              </div>
              <span style={{fontSize:18,color:grayC,marginLeft:8}}>{open?'▲':'▼'}</span>
            </button>
            {open&&(
              <div style={{borderTop:`1px solid ${divider}`,padding:'12px 16px 16px'}}>
                {c.phone&&<a href={`tel:${c.phone}`} style={{fontSize:13,color:'#2d8a60',textDecoration:'none',display:'block',marginBottom:8}}>{c.phone}</a>}
                {c.notes&&<p style={{fontSize:12,color:grayC,fontStyle:'italic',marginBottom:10}}>{c.notes}</p>}
                <p style={{fontSize:11,fontWeight:700,color:grayC,textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:8}}>Archivos</p>
                {clientFiles.length===0&&<p style={{fontSize:12,color:grayC,marginBottom:10}}>Sin archivos aún</p>}
                {clientFiles.map(f=>(
                  <div key={f.name} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'8px 10px',background:subtleBg,borderRadius:8,marginBottom:6}}>
                    <a href={`/api/admin/files?user_id=${c.id}&file=${encodeURIComponent(f.name)}`} target="_blank" rel="noreferrer" style={{fontSize:12,color:inkC,textDecoration:'none',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',flex:1}}>{f.name.replace(/^\d+_/,'')}</a>
                    {userRole==='admin'&&<button onClick={()=>deleteFile(c.id,`clients/${c.id}/${f.name}`)} style={{flexShrink:0,marginLeft:8,background:'none',border:'none',color:'#c0392b',cursor:'pointer',fontSize:16,padding:0}}>✕</button>}
                  </div>
                ))}
                <label style={{display:'flex',alignItems:'center',justifyContent:'center',gap:8,padding:'10px',background:'rgba(184,151,90,0.08)',border:`1px dashed rgba(184,151,90,0.4)`,borderRadius:10,cursor:'pointer',marginTop:4}}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#b8975a" strokeWidth="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                  <span style={{fontSize:12,color:'#b8975a',fontWeight:600}}>{uploading[c.id]?'Subiendo...':'Subir archivo'}</span>
                  <input type="file" style={{display:'none'}} disabled={uploading[c.id]} onChange={e=>{if(e.target.files[0])upload(c.id,e.target.files[0])}}/>
                </label>
              </div>
            )}
          </div>
...
// ─────────────────────────────────────────────
// DOCUMENTOS — shared contracts with word search
// ─────────────────────────────────────────────
function DocumentosPanel({userRole,showToast}){
  const {dm,inkC,grayC,cardBg,cardBorder,inputBg,inputBorder,subtleBg,divider}=useTheme()
  ...
  async function upload(file){
    ...
    if(r.ok){showToast('Documento subido');load()}
    else showToast('Error al subir')
    setUploading(false)
  }
  ...
  const filtered=docs.filter(f=>f.name.toLowerCase().replace(/^\d+_/,'').replace(/[_-]/g,' ').includes(q.toLowerCase()))
  function prettyName(name){return name.replace(/^\d+_/,'').replace(/[_-]/g,' ').replace(/\.[^.]+$/,'')}
  function ext(name){return(name.match(/\.([^.]+)$/)||['',''])[1].toUpperCase()}
  const extColor={'PDF':'#e74c3c','DOC':'#2980b9','DOCX':'#2980b9','XLS':'#27ae60','XLSX':'#27ae60','PNG':'#8e44ad','JPG':'#8e44ad','JPEG':'#8e44ad'}
  return(
    <div style={{padding:'20px 16px 100px',fontFamily:ff2}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
        <h1 style={{fontFamily:`'Cormorant Garamond',serif`,fontSize:26,fontWeight:300,color:inkC}}>Documentos</h1>
        {userRole==='admin'&&(
          <label style={{display:'flex',alignItems:'center',gap:6,padding:'7px 14px',background:'rgba(184,151,90,0.1)',border:'1px solid rgba(184,151,90,0.3)',borderRadius:99,cursor:'pointer'}}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#b8975a" strokeWidth="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
            <span style={{fontSize:12,color:'#b8975a',fontWeight:600,whiteSpace:'nowrap'}}>{uploading?'Subiendo...':'Subir'}</span>
            <input type="file" style={{display:'none'}} disabled={uploading} onChange={e=>{if(e.target.files[0])upload(e.target.files[0])}}/>
          </label>
        )}
      </div>
      <p style={{fontSize:12,color:grayC,marginBottom:16}}>Contratos, plantillas y documentos del equipo</p>
      <div style={{position:'relative',marginBottom:16}}>
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={grayC} strokeWidth="2" style={{position:'absolute',left:12,top:'50%',transform:'translateY(-50%)'}}><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
        <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Buscar por nombre..." style={{width:'100%',boxSizing:'border-box',height:40,borderRadius:10,border:`1px solid ${inputBorder}`,padding:'0 12px 0 36px',fontSize:13,fontFamily:ff2,background:inputBg,color:inkC,outline:'none'}}/>
      </div>
      {loading&&<p style={{textAlign:'center',color:grayC,padding:'40px 0',fontSize:13}}>Cargando...</p>}
      {!loading&&filtered.length===0&&<p style={{textAlign:'center',color:grayC,padding:'40px 0',fontSize:13}}>{q?'Sin resultados para "'+q+'"':'Sin documentos aún'}</p>}
      {filtered.map(f=>{
        const e=ext(f.name)
        const color=extColor[e]||'#636e72'
        return(
          <div key={f.name} style={{display:'flex',alignItems:'center',gap:12,background:cardBg,border:`1px solid ${cardBorder}`,borderRadius:12,padding:'12px 14px',marginBottom:8}}>
            <span style={{flexShrink:0,fontSize:10,fontWeight:700,padding:'4px 7px',borderRadius:6,background:`${color}18`,color}}>{e||'—'}</span>
            <a href={`/api/admin/documents?file=${encodeURIComponent(f.name)}`} target="_blank" rel="noreferrer" style={{flex:1,fontSize:13,color:inkC,textDecoration:'none',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',textTransform:'capitalize'}}>{prettyName(f.name)}</a>
            {userRole==='admin'&&<button onClick={()=>del(f.name)} style={{flexShrink:0,background:'none',border:'none',color:'#c0392b',cursor:'pointer',fontSize:18,padding:0,lineHeight:1}}>✕</button>}
          </div>
        )
      })}
    </div>
...
export default function Admin({session}){
  ...
  return(
    ...
    <>
      ...
      {/* ── GLASS BOTTOM NAV ── */}
      <nav style={{position:'fixed',bottom:0,left:0,right:0,zIndex:200,height:'calc(64px + env(safe-area-inset-bottom,0px))',paddingBottom:'env(safe-area-inset-bottom,0px)',background:darkMode?'rgba(20,20,18,0.88)':'rgba(255,255,255,0.82)',backdropFilter:'blur(24px) saturate(180%)',WebkitBackdropFilter:'blur(24px) saturate(180%)',borderTop:`1px solid ${darkMode?'rgba(255,255,255,0.08)':'rgba(255,255,255,0.5)'}`,boxShadow:'0 -2px 20px rgba(0,0,0,0.10)',display:'flex',alignItems:'flex-start',paddingTop:8}}>
        {/* Dashboard */}
        {(()=>{const id='dashboard';const active=activePanel===id;return(
          <button key={id} onClick={()=>{setActivePanel(id);setTab(id);setBurger(false)}} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:3,border:'none',background:'none',cursor:'pointer',padding:'4px 0',touchAction:'manipulation'}}>
            <span style={{color:active?gold:darkMode?'rgba(255,255,255,0.3)':'rgba(14,14,12,0.3)',transition:'color 0.15s'}}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg></span>
            <span style={{fontSize:10,fontFamily:ff,fontWeight:active?700:400,color:active?gold:darkMode?'rgba(255,255,255,0.35)':'rgba(14,14,12,0.35)',letterSpacing:'0.02em',borderBottom:active?`2px solid ${gold}`:'2px solid transparent',paddingBottom:1}}>Dashboard</span>
          </button>
        )})()}
        {/* Consultas */}
        {(()=>{const id='bookings';const active=activePanel===id;return(
          <button key={id} onClick={()=>{setActivePanel(id);setTab(id);setBurger(false)}} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:3,border:'none',background:'none',cursor:'pointer',padding:'4px 0',touchAction:'manipulation'}}>
        ...
        {/* Llamadas */}
        {(()=>{const id='coldcalling';const active=activePanel===id;return(
          <button key={id} onClick={()=>{setActivePanel(id);setTab(id);setBurger(false)}} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:3,border:'none',background:'none',cursor:'pointer',padding:'4px 0',touchAction:'manipulation'}}>
            <span style={{color:active?gold:darkMode?'rgba(255,255,255,0.3)':'rgba(14,14,12,0.3)',transition:'color 0.15s'}}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.65 3.38 2 2 0 0 1 3.62 1h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.58a16 16 0 0 0 6 6l.96-.87a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 21.92 16.92z"/></svg></span>
            <span style={{fontSize:10,fontFamily:ff,fontWeight:active?700:400,color:active?gold:darkMode?'rgba(255,255,255,0.35)':'rgba(14,14,12,0.35)',letterSpacing:'0.02em',borderBottom:active?`2px solid ${gold}`:'2px solid transparent',paddingBottom:1}}>Llamadas</span>
          </button>
        )})()}
        {/* Ver Más burger — all roles */}
        <button onClick={()=>setBurger(o=>!o)} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:3,border:'none',background:'none',cursor:'pointer',padding:'4px 0',touchAction:'manipulation'}}>
          <span style={{color:burger?gold:darkMode?'rgba(255,255,255,0.3)':'rgba(14,14,12,0.3)',transition:'color 0.15s'}}>
            {burger
              ?<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
              :<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="18" x2="21" y2="18"/></svg>
            }
          </span>
          <span style={{fontSize:10,fontFamily:ff,fontWeight:burger?700:400,color:burger?gold:darkMode?'rgba(255,255,255,0.35)':'rgba(14,14,12,0.35)',letterSpacing:'0.02em',borderBottom:burger?`2px solid ${gold}`:'2px solid transparent',paddingBottom:1}}>Ver Más</span>
        </button>
      </nav>
      ...
      {burger&&(
        <>
          ...
          <div style={{position:'fixed',bottom:0,left:0,right:0,zIndex:195,background:darkMode?'rgba(22,22,20,0.96)':'rgba(255,255,255,0.92)',backdropFilter:'blur(32px) saturate(200%)',WebkitBackdropFilter:'blur(32px) saturate(200%)',borderRadius:'24px 24px 0 0',border:`1px solid ${darkMode?'rgba(255,255,255,0.08)':'rgba(255,255,255,0.6)'}`,boxShadow:'0 -8px 40px rgba(0,0,0,0.18)',paddingBottom:'calc(env(safe-area-inset-bottom,0px) + 8px)',animation:'sheet-up 0.28s cubic-bezier(0.22,1,0.36,1) both'}}>
            <div style={{width:40,height:4,background:darkMode?'rgba(255,255,255,0.15)':'rgba(14,14,12,0.15)',borderRadius:99,margin:'12px auto 16px'}}/>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,padding:'0 16px 8px'}}>
              {[
                ...
                <button key={id} onClick={()=>{setActivePanel(id);setPanel(id);setBurger(false)}} style={{display:'flex',alignItems:'center',gap:10,padding:'13px 14px',background:activePanel===id?(darkMode?'rgba(255,255,255,0.1)':'rgba(14,14,12,0.07)'):(darkMode?'rgba(255,255,255,0.04)':'rgba(14,14,12,0.03)'),border:'1px solid',borderColor:activePanel===id?gold:(darkMode?'rgba(255,255,255,0.08)':'rgba(14,14,12,0.06)'),borderRadius:14,fontFamily:ff,fontSize:13,fontWeight:activePanel===id?600:400,color:darkMode?'rgba(255,255,255,0.9)':ink,cursor:'pointer',textAlign:'left',touchAction:'manipulation'}}>
                  <span style={{display:'flex',alignItems:'center',color:activePanel===id?gold:darkMode?'rgba(255,255,255,0.45)':gray,flexShrink:0}}>{icon}</span>{label}
                </button>
              ))}
            </div>
            <div style={{height:1,background:darkMode?'rgba(255,255,255,0.07)':'rgba(14,14,12,0.07)',margin:'8px 16px'}}/>
            <button onClick={signOut} style={{display:'block',width:'100%',padding:'14px 20px',background:'none',border:'none',textAlign:'center',fontFamily:ff,fontSize:13,color:darkMode?'rgba(255,255,255,0.4)':gray,cursor:'pointer'}}>
              Cerrar sesión
            </button>
          </div>
        </>
